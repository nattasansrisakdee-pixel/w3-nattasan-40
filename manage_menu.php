...
<title>จัดการเมนู | FlameMenu</title>
...
<body>
    ...
    <main class="page-container">
        ...
        <div class="breadcrumb">
            ...
            <span>/</span>
            <span class="crumb-current">จัดการเมนู</span>
        </div>
        ...
        $menus = $result ? mysqli_fetch_all($result, MYSQLI_ASSOC) : [];
        $total = count($menus);

        $typeResult = mysqli_query($con, "SELECT * FROM menu_types");
        $totalTypes = $typeResult ? mysqli_num_rows($typeResult) : 0;

        $prices = array_map(fn($m) => (float)$m['menu_price'], $menus);
        ...
        <div class="wrap">
            <div class="dash-header">
                <div class="dash-title">
                    <div class="section-eyebrow">Admin Panel</div>
                    <h1>จัดการ<span class="accent">เมนูอาหาร</span></h1>
                    <p>เพิ่ม แก้ไข หรือลบรายการเมนูทั้งหมดของร้าน</p>
                </div>
                <a href="add_menu.php" class="btn btn-flame btn-sm">+ เพิ่มเมนูใหม่</a>
            </div>

            <div class="kpi-row">
                <div class="kpi-card">
                    <div class="kpi-num"><?= $total ?></div>
                    <div class="kpi-label">เมนูทั้งหมด</div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-num"><?= $totalTypes ?></div>
                    <div class="kpi-label">หมวดหมู่ทั้งหมด</div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-num">฿<?= number_format($avgPrice, 0) ?></div>
                    <div class="kpi-label">ราคาเฉลี่ยต่อเมนู</div>
                </div>
            </div>

            <div class="search-wrap">
                <span class="search-icon">🔍</span>
                <input type="text" id="menuSearch" placeholder="ค้นหาเมนูจากชื่อหรือรหัส..." onkeyup="filterMenus()">
            </div>
            ...
            <div class="table-card">
                <table class="data-table" id="menuTable">
                    ...
                    <tbody>
                        ...
                        <tr data-search="<?= strtolower($menu["menu_id"] . " " . $menu["menu_name"]) ?>">
                            <td><span class="id-pill"><?= $menu["menu_id"] ?></span></td>
                            <td><?= $menu["menu_name"] ?></td>
                            <td class="price-cell"><?= $menu["menu_price"] ?></td>
                            <td><img class="table-thumb" src="<?= $menu["menu_image"] ?>" alt="<?= $menu["menu_name"] ?>"></td>
                            <td><span class="type-pill">ประเภท <?= $menu["type_id"] ?></span></td>
                            <td>
                                <div class="action-cell">
                                    <a href="edit_menu.php?id=<?= $menu["menu_id"] ?>" class="btn btn-gold btn-sm">แก้ไข</a>
                                    <a href="action/delete_menu.php?id=<?= $menu["menu_id"] ?>" class="btn btn-danger-ghost btn-sm" onclick="return confirm('ยืนยันการลบเมนูนี้หรือไม่?');">ลบ</a>
                                </div>
                            </td>
                        </tr>
                        <?php } ?>
                ...
                <div class="no-results" id="noResults">ไม่พบเมนูที่ค้นหา</div>
            </div>
        </div>
