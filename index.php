...
<head>
    ...
    <title>FlameMenu | เมนูอาหารทั้งหมด</title>
...
<body>
    <header class="site-header">
        <div class="site-header-inner">
            ...
            <nav class="site-nav">
                ...
                <a href="manage_menu.php" class="nav-link">จัดการเมนู</a>
            </nav>
        </div>
    </header>
    ...
    <main class="page-container">
        ...
        <?php
            ...
            // ดึงเมนูพร้อมชื่อประเภท (join กับ menu_types)
            ...
            // ดึงรายการประเภททั้งหมด สำหรับแท็บกรอง
            $typeResult = mysqli_query($con, "SELECT * FROM menu_types");
            $types = $typeResult ? mysqli_fetch_all($typeResult, MYSQLI_ASSOC) : [];

            // สถิติสำหรับ hero
            $totalMenus = count($menus);
            $totalTypes = count($types);
            $prices = array_map(fn($m) => (float)$m['menu_price'], $menus);
            $startPrice = count($prices) ? min($prices) : 0;
        ?>

        <section class="hero">
            <div class="wrap hero-inner">
                <div class="hero-eyebrow">🔥 Est. Flavor Since Day One</div>
                <h1>อิ่มอร่อยแบบ<span class="accent">จัดเต็ม</span><br>ทุกเมนูที่คุณคิดถึง</h1>
                <p class="hero-sub">คัดสรรเมนูไก่ทอดกรอบ เบอร์เกอร์ และของว่างสุดฟิน พร้อมเสิร์ฟความอร่อยถึงมือคุณทุกวัน</p>
                <div class="hero-actions">
                    <a href="#menu" class="btn btn-flame">ดูเมนูทั้งหมด</a>
                    <a href="manage_menu.php" class="btn btn-outline">สำหรับผู้ดูแลระบบ</a>
                </div>
            </div>
        </section>
        ...
        <section class="menu-section" id="menu">
            <div class="wrap">
                ...
                <div class="section-head">
                    <div class="section-eyebrow">Full Menu</div>
                    <h2>เลือกเมนูที่<span class="accent">ใช่</span>สำหรับคุณ</h2>
                    <p class="section-sub">กรองตามหมวดหมู่ด้านล่างเพื่อค้นหาเมนูที่ต้องการได้รวดเร็วขึ้น</p>
                </div>

                <div class="filter-bar" id="filterBar">
                    <button class="filter-chip active" data-filter="all">ทั้งหมด</button>
                    <?php foreach($types as $type){ ?>
                        <button class="filter-chip" data-filter="<?= $type['type_id'] ?>"><?= $type['type_name'] ?></button>
                    <?php } ?>
                </div>

                <div class="menu-grid" id="menuGrid">
                    <?php foreach($menus as $menu){ ?>
                        <div class="menu-card" data-type="<?= $menu['type_id'] ?>">
                            <div class="menu-card-media">
                                <span class="menu-card-id"><?= $menu["menu_id"] ?></span>
                                <img src="<?= $menu["menu_image"] ?>" alt="<?= $menu["menu_name"] ?>">
                            </div>
                            <div class="menu-card-body">
                                <div class="menu-card-type"><?= $menu["type_name"] ?? 'เมนูทั่วไป' ?></div>
                                <p class="menu-card-name"><?= $menu["menu_name"] ?></p>
                ...
                <div class="empty-state" id="emptyState">
                    ...
                    <div>ไม่พบเมนูในหมวดหมู่นี้</div>
                </div>
